        ...
        <table class="rwd-table" style="background-color: black; border: 1px solid white">
            ...
            <tbody>
                <tr v-for="reservation in nextReservationsSanja" v-bind:key="reservation.id">
                    ...
                    <td><button
                            @click="cancelReservationSanja(reservation.code)" class="button is-danger">Otkaži
                    </button></td>
                </tr>
            </tbody>
        ...
        methods: {
            ...
            async cancelReservationSanja(doc) {
                if(confirm('Jesi li siguran?')) { 
                    let bookingRef = await db.collection('bookingsSanja').where('code', '==', doc).get();
                    let idBooking = bookingRef.docs[0].id;
                    await db.collection('bookingsSanja').doc(idBooking).delete()
                    let detailRef = await db.collection('detailsSanja').where('code', '==', doc).get();
                    let id = detailRef.docs[0].id;
                    await db.collection('detailsSanja').doc(id).delete();
                    this.watcher();
                    this.readData();
                }else {
            ...
            readData() {
                db.collection("detailsSanja").where('date', '>', new Date()).get().then((querySnapshot) => {
